...
public abstract class AssociatedObject {
    ...
    public void unlink(AssociatedObject target, String id) throws AssociationDoesNotExistException, AssociationMultiplicityException {
        unlink(target, id, 2);
    }

    public void unlink(AssociatedObject target) throws AssociationDoesNotExistException, AssociationMultiplicityException {
        unlink(target, DEFAULT_ASSOCIATION_ID, 2);
    }

    private void unlink(AssociatedObject target, String id, int callCounter) throws AssociationDoesNotExistException, AssociationMultiplicityException {
        if (callCounter <= 0)
            return;
